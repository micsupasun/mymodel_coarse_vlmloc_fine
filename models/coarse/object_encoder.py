...
class ObjectEncoder(torch.nn.Module):
    def __init__(self, embed_dim: int, known_classes: List[str], known_colors: List[str], args):
        ...
        if args.pointnet_path:
            ...
            self.pointnet.load_state_dict(pointnet_state, strict=True)
        elif not getattr(args, "allow_checkpoint_pointnet_init", False):
            raise ValueError(
                "pointnet_path is empty. Set a matching standalone PointNet checkpoint, "
                "or set allow_checkpoint_pointnet_init only when the enclosing model "
                "checkpoint will immediately be audited and loaded."
            )
